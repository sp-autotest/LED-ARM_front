<template>
    <div class="bc-type">
        <div class="headline">
            <h3 class="headline-text">Тип ВС</h3>
        </div>
        <div class="search-add-bc-type">
            <div class="add-bc-type">
                <router-link tag="span" class="router-link" to="/control/bc-type/add">
                    <cButton buttonValue="Добавить Тип ВС"></cButton>
                </router-link>
            </div>
            <div class="search-bc-type">
                <v-text-field
                        v-model="search"
                        append-icon="search"
                        label="Search"
        ...
        <div class="table-wrap">
            ...
            >
                ...
                <template slot="items" scope="props">
                    <td @click="editBCType(props.item.id)">{{props.item.name_ru}}</td>
                    <td @click="editBCType(props.item.id)">{{props.item.name_eng}}</td>
                    <td @click="editBCType(props.item.id)">{{props.item.aircraft_class_code}}</td>
                    <td @click="editBCType(props.item.id)">{{props.item.ccp}}</td>
                </template>
            </v-data-table>
    ...
    export default {
        ...
        data() {
            return {
                ...
                headers: [
                    {text: 'Название полное', align: 'left', value: 'name_ru'},
                    {text: 'Название полное (англ.)', align: 'left', value: 'name_eng'},
                    {text: 'Код класса ВС', align: 'left', value: 'aircraft_class_code'},
                    {text: 'Компоновка раздельно первый / бизнес / эконом', align: 'left', value: 'ccp'},
                ],
                feesData: [],
            }
        },
        mounted() {
            this.$store.dispatch('controls/' + types.A_BC_TYPE)
                .then( (res) => {
                    this.bcTypeData = res.data.bctypes
                })
        },
        methods: {
            editBCType(id) {
                this.$router.push('/control/bc-type/add/' + id)
            }
        },
        computed: {
            rowPerPageSet: {
