<template>
    <div class="provider">
        <div class="headline">
            <h3 class="headline-text">Поставщики</h3>
        </div>
        <div class="search-add-provider">
            <div class="add-provider">
                <router-link tag="span" class="router-link" to="/control/provider/add">
                    <cButton buttonValue="Добавить поставщика"></cButton>
                </router-link>
            </div>
        ...
        <div class="table-wrap">
            ...
            >
                ...
                <template slot="items" scope="props">
                    <td @click="editProvider(props.item.id)">{{props.item.name_ru}}</td>
                    <td @click="editProvider(props.item.id)">{{props.item.name_full_ru}}</td>
                    <td @click="editProvider(props.item.id)">{{props.item.name_eng}}</td>
                    <td @click="editProvider(props.item.id)">{{props.item.name_full_eng}}</td>
                    <td @click="editProvider(props.item.id)">{{props.item.date_begin_at}}</td>
                    <td @click="editProvider(props.item.id)">{{props.item.date_end_at}}</td>
                </template>
            </v-data-table>
    ...
    export default {
        ...
        data() {
            return {
                ...
                headers: [
                    {text: 'Название', align: 'left', sortable: true, value: 'name_ru'},
                    {text: 'Полное название', align: 'left', value: 'name_full_ru'},
                    {text: 'Название (англ)', align: 'left', value: 'name_eng'},
                    {text: 'Полное название (англ)', align: 'left', value: 'name_full_eng'},
        ...
        mounted() {
            this.$store.dispatch('controls/' + types.A_PROVIDER)
                .then( (res) => {
                    console.log(res)
                    this.providerData = res.data.providers
                })
        },
        methods: {
            editProvider(id) {
                this.$router.push('/control/provider/add/' + id)
            },
        },
        computed: {
            rowPerPageSet: {
            ...
            pages () {
                ...
                if(this.providerData != undefined) {
                    countLength = this.providerData.length;
                }
